...
void main( void )
{
	...
	finalColor += CalculateDirLight(dirLight, norm, eyeDirection);
...
vec3 CalculateSpotLight(SpotLight pLight, vec3 pNormal, vec3 pFragPos, vec3 pEyeDir)
{
	...
	float attenuation = 1.0f / (pLight.constAtten + pLight.linearAtten * dist + pLight.quadAtten * pow(dist, 2));
	//spotlight
	float theta = dot(lightDir, normalize(-pLight.spotDirection)); 	//the angle(cos) between the spot direction and the light to pixel direction
