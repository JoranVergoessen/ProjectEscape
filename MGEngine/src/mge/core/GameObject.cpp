...
void GameObject::SetDirection(glm::vec3 pNewDirection)
{
	/**
	glm::vec3 forward = glm::normalize(pNewDirection);
	...
	setTransform(
		glm::mat4(	glm::vec4(right, 0),
					glm::vec4(up, 0),
					glm::vec4(forward, 0),
					glm::vec4(getLocalPosition(), 1)));
	/**/

	setTransform(
		glm::inverse(
			glm::lookAt(getWorldPosition() + glm::normalize(pNewDirection), getWorldPosition(), glm::vec3(0, 1, 0))
			)
		);
}

//gets the forward vector of the transform matrix (z)
glm::vec3 GameObject::GetDirection() const
{
...
//rotate object pAngle(in radians)
void GameObject::rotate(float pAngleRad, glm::vec3 pAxis)
{
	setTransform(glm::rotate(_transform, pAngleRad, pAxis));
}
