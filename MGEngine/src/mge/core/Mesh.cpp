...
vector<glm::vec2> uvs;
map <Mesh::FaceIndexTriplet, unsigned int> mappedTriplets;
	...
	if( file.is_open() ){
		...
		while(getline(file,line)) {
            ...
            //their own buffer
			    ...
			    //a triangular face looks like
			    //f 2/1/1 1/2/1 3/3/1				OR				f 2/1 2/1 2/1				OR				f 2//1 2//1 2//1
			    //f v1/u1/n1 v2/u2/n2 v3/u3/n3		OR				f v1/u1 v2/u2 v3/u3			OR				f v1//n1 v2//n2 v3//n3

				//a quad face looks like
				//f  2/ 1/1   1/ 2/1   3/ 3/1   1/ 2/3		OR		f  2/1    2/1    2/1    1/2			OR				f  2//1   2//1   2//1   1//2
				//f v1/u1/n1 v2/u2/n2 v3/u3/n3 v4/u4/n4		OR		f v1/u1  v2/u2  v3/u3  v4/u4		OR				f v1//n1 v2//n2 v3//n3 v4//n4
				...
				glm::ivec3 t1VertexIndex;
				glm::ivec3 t1NormalIndex;
				glm::ivec3 t1UvIndex;
				glm::ivec3 t2VertexIndex;
				glm::ivec3 t2NormalIndex;
				glm::ivec3 t2UvIndex;
				//											1		2		3		4
				int count = sscanf(line.c_str(), "%10s %d/%d/%d %d/%d/%d %d/%d/%d %d/%d/%d", cmd,
					&t1VertexIndex[0], &t1UvIndex[0], &t1NormalIndex[0],	//t1 index 0 + t2 index 0
					&t1VertexIndex[1], &t1UvIndex[1], &t1NormalIndex[1],	//t1 index 1
					&t1VertexIndex[2], &t1UvIndex[2], &t1NormalIndex[2],	//t1 index 2 + t2 index 1
					&t2VertexIndex[2], &t2UvIndex[2], &t2NormalIndex[2]);	//			   t2 index 2

				//copy the missing information for t2
				t2VertexIndex[0] = t1VertexIndex[0];
				t2VertexIndex[1] = t1VertexIndex[2];
				t2UvIndex[0] = t1UvIndex[0];
				t2UvIndex[1] = t1UvIndex[2];
				t2NormalIndex[0] = t1NormalIndex[0];
				t2NormalIndex[1] = t1NormalIndex[2];

				//if we have 10 elements its a normal tri face, so just parse it
				if ( count == 10 ) 
				{
					//cout << "parsing a v1/u1/n1 triangular face" << endl;
					parseVunTriFace(t1VertexIndex, t1NormalIndex, t1UvIndex, mesh);
				}
				//if we have 13 elements its a quad face, we need to parse it as 2 separate triangles
				else if(count == 13)
				{
					//if we have quad faces instead of triangular faces we need to split the quad face into 2 triangular faces
					//cout << "trying to read quad face from line [" << line << "] count is: " << count << endl;

					//now we need to parse both triangles instead of the one quad
					parseVunTriFace(t1VertexIndex, t1NormalIndex, t1UvIndex, mesh);
					parseVunTriFace(t2VertexIndex, t2NormalIndex, t2UvIndex, mesh);
				...
				else 
				{
					...
					cout << "Error in line [" << line << "] : counted " << count << "   ";
					//cout << " counted " << count << line<<" ( " << vertexIndex[0] << "  " << uvIndex[0] << " )... ";
...
void Mesh::parseVunTriFace(glm::ivec3 pVertexIndex, glm::ivec3 pNormalIndex, glm::ivec3 pUvIndex, Mesh* pMesh)
{
	...
	for (int i = 0; i < 3; ++i) {
		...
		FaceIndexTriplet triplet(pVertexIndex[i], pUvIndex[i], pNormalIndex[i]);
		...
		if (found == mappedTriplets.end())
		{
			...
			pMesh->_indices.push_back(index);
			...
			pMesh->_vertices.push_back(vertices[pVertexIndex[i] - 1]);
			pMesh->_normals.push_back(normals[pNormalIndex[i] - 1]);
			pMesh->_uvs.push_back(uvs[pUvIndex[i] - 1]);
		}
		else
		{
